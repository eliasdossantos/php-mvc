...
abstract class Controller
{
     ...
     * dinâmico (ex: nome de view montado a partir de parâmetro de rota) em
    ...
    {
        $url = str_starts_with($url, 'http') ? $url : rtrim($this->appUrl(), '/') . '/' . ltrim($url, '/');
    ...
    {
        $this->redirect($_SERVER['HTTP_REFERER'] ?? $this->appUrl());
    }
    ...
    protected function validate(array $data, array $rules, string $redirectBack = ''): array
    {
        ...
        if ($validator->fails()) {
            ...
            $back = $redirectBack ?: ($_SERVER['HTTP_REFERER'] ?? $this->appUrl());
            $this->redirect($back);
        }
